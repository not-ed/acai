...
namespace AcaiCore
{
    public class FoodItemTableSchema : IJournalTableSchema
    {
        ...
        public bool PresentInConnection(SqliteConnection sqliteConnection)
        {
            ...
            var createdAtColumnIsPresent = false;
            ...
            using (var selectTableCommand = sqliteConnection.CreateCommand())
            {
                ...
                using (var reader = selectTableCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        if (!createdAtColumnIsPresent && CreatedAtColumnIsPresentInSchema(reader))
                        {
                            createdAtColumnIsPresent = true;
                        }
                    }
                }
            }

            return idColumnIsPresent && nameColumnIsPresent && caloriesColumnIsPresent && createdAtColumnIsPresent;
        }
        ...
        private bool CreatedAtColumnIsPresentInSchema(SqliteDataReader reader)
        {
            if (reader.GetInt64(0) != 3) return false; // cid
            if (reader.GetString(1) != "created_at") return false; // name
            if (reader.GetString(2) != "TEXT") return false; // type
            if (reader.GetInt64(3) != 1) return false; // notnull
            if (reader.GetInt64(5) != 0) return false; // pk
